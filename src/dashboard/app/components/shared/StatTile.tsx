...
export function StatTile({ label, value, sub, variant = "default", accent, size = "lg", corner, children }: StatTileProps) {
  ...
  const labelClass = variant === "inverted" ? "wd-label-inverted" : "wd-label-muted";
  return (
    <div className={`${cardClass} p-5 flex flex-col justify-between gap-4 min-h-[132px]`}
      ...
      <div className="flex items-start justify-between gap-2">
        <span className={`wd-label ${labelClass}`}>{label}</span>
        {corner}
      </div>
      <div>
        ...
        {sub && <p className={`wd-label mt-2 ${labelClass}`}>{sub}</p>}
        {children}
      </div>
    </div>
  );
