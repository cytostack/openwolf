...
function copyHookScripts(wolfDir: string): void {
  ...
  // __dirname at runtime is dist/src/cli/, so the compiled hooks sit next door.
  ...
  if (sourceDir) {
    ...
    const tsModules = fs.readdirSync(srcHooksDir).filter((f) => f.endsWith(".ts")).sort();
    for (const tsFile of tsModules) {
      ...
      fs.writeFileSync(path.join(hooksDir, tsFile.replace(".ts", ".js")), loaderContent, "utf-8");
      copiedAny = true;
    }
  }
